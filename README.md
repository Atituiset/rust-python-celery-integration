...
| 组件 | 技术栈 | 职责 |
|------|--------|------|
| `rust-producer` | Rust, tokio, redis-rs | 构造 Celery v2 消息体 LPUSH 到 DB 6；单一 Pub/Sub 长连接订阅 DB 7 `set` 事件，通过 `oneshot` 内存通道精确唤醒任务协程 |
...
├── rust-producer/
│   ├── Cargo.toml              # tokio, redis, serde, uuid, base64, futures-util
│   └── src/main.rs             # push_task() + ResultListener (Pub/Sub 事件总线)
└── README.md                   # 本文件
```
